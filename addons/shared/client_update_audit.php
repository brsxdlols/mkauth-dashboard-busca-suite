...
function mka_client_audit_ensure_table($conn)
{
    ...
    foreach (array('previous_last_update' => 'DATETIME NULL', 'captured_at' => 'DATETIME NULL') as $column => $definition) {
...
if (isset($_POST['mka_client_audit']) && $_POST['mka_client_audit'] === 'prepare') {
    ...
    if ($usuario !== '' && ($uuid !== '' || $login !== '') && $conn instanceof mysqli) {
        ...
        if ($query && ($row = mysqli_fetch_assoc($query))) {
            ...
            @mysqli_query($conn, "INSERT INTO dashboard_am_client_update_audit (uuid_cliente,login_cliente,usuario,previous_last_update,captured_at)
                VALUES ('" . $uuid_sql . "','" . $login_sql . "','" . $usuario_sql . "'," . $previous_sql . ",NOW())
                ON DUPLICATE KEY UPDATE login_cliente=VALUES(login_cliente),usuario=VALUES(usuario),previous_last_update=VALUES(previous_last_update),captured_at=VALUES(captured_at)");
        }
        @mysqli_close($conn);
    }
