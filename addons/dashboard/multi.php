...
if ($canTotals) {
    ...
    if (!$multiError) {
        ...
        $cards=array(
            ...
            array('Sem carnê',(int)$counts['no_booklet'],$total,'sem carne','light'),
            array('Sem títulos',(int)$counts['no_titles'],$total,'sem tit','light')
        );
    }
}
?>
...
</style><style>@media(min-width:1101px){.multi-cards{grid-template-columns:repeat(4,minmax(0,1fr));}}</style></head><body class="multi-page mka-suite-dashboard-page">
