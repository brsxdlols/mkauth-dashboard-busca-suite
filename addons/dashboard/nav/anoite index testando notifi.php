    ...
    <div class="col-auto">
        <a href="dash_notificacoes.php" onclick="window.open(this.href, this.target, 'width=650, height=700, scrollbars=yes'); return false;">
            <?php if ($num_notificacoes > 0): ?>
                ...
                <span class="badge badge-danger"><?php echo $num_notificacoes; ?></span>
            <?php else: ?>
                <!-- Exibe '0' quando não há notificações com texto preto -->
                <span class="badge badge-secondary" style="color: black;"><?php echo 0; ?></span>
            <?php endif; ?>
        </a>
    </div>
</div>
        ...
        // Card Clientes
        // echo $grupos;
        $query_clientes_online = mysqli_query($conn, "SELECT r.username FROM radacct r LEFT JOIN sis_cliente c ON c.login = r.username WHERE $grupos r.acctstoptime IS NULL ");
                            ...
                            <div class="col">
                                <div class="card text-bg-light">
                                    <div class="card-header">
                                     <a href="chamados_abertos.php" class="stretched-link">Chamados</a>
                                    </div>
                                    <div class="card-body m-0 p-0">
                        ...
                        <div class="card-body m-0 p-0">
                            ...
                            <div class="row g-1">
                                ...
                                foreach ($ramal_cliente as $key => $value) {
                                    ...
                                    $queryOnline = mysqli_query($conn, "SELECT username FROM radacct r LEFT JOIN sis_cliente c ON r.username = c.login WHERE c.ramal LIKE '$key' AND c.cli_ativado LIKE 's' AND r.acctstoptime IS NULL");

                                    $queryAdicionaisOnline = mysqli_query($conn, "SELECT r.username FROM radacct r LEFT JOIN sis_adicional cli_add ON r.username = cli_add.username LEFT JOIN sis_cliente c ON cli_add.login = c.login WHERE cli_add.ramal LIKE '$key' AND c.cli_ativado LIKE 's' AND r.acctstoptime IS NULL");
                ...
                <div class='col-12 mb-3'>
                    <table class='table table-sm'>
                        ...
                        <tbody>
                            ...
                            $query_sis_instalacao = mysqli_query($conn, "SELECT * FROM sis_solic WHERE status = 'aberto' AND (datainst <= '$nova_data_1' OR visita <= '$nova_data_1' OR datainst IS NULL) ORDER BY datainst");
