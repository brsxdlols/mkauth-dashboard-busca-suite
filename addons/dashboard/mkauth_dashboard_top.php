...
@media screen and (max-width: 1023px) {
    ...
    #mkauth-dashboard-navbar .navbar-dropdown .columns {
        display: block;
    }
    #mkauth-dashboard-navbar .navbar-dropdown .is-divisor-vertical {
        display: none;
    }
}</style>
<script>
function dashboardMobileMenuToggle(event) {
    if (event) {
    ...
    var opener = document.getElementById('navbar-mka');
...
<nav id="mkauth-dashboard-navbar" class="navbar is-fixed-top <?php echo $dashTopMenuClass; ?>" role="navigation" aria-label="dropdown navigation">
    <div class="navbar-brand">
        <a class="navbar-item is-hidden-desktop" id="navbar-mka" href="javascript:void(0)" data-target="navMenu" onclick="return dashboardMobileMenuToggle(event);"><i class="bi-list"></i></a>
...
<script>
(function () {
    function bindDashboardMobileMenu() {
        var navbar = document.getElementById('mkauth-dashboard-navbar');
        if (!navbar) return;

        var opener = document.getElementById('navbar-mka') || navbar.querySelector('.navbar-burger');
        var menu = document.getElementById('navMenu');

        if (opener && menu) {
            opener.setAttribute('data-dashboard-bound', '1');
        }

        var links = navbar.querySelectorAll('.has-dropdown > .navbar-link, .has-dropdown > div.navbar-link');
        links.forEach(function (link) {
            if (link.getAttribute('data-dashboard-bound') === '1') return;
            link.setAttribute('data-dashboard-bound', '1');
    ...
    if (document.readyState === 'loading') {
        document.addEventListener('DOMContentLoaded', bindDashboardMobileMenu);
    } else {
        bindDashboardMobileMenu();
    }

    window.addEventListener('load', bindDashboardMobileMenu);
