...
<?php include('../../topo.php'); ?>
    ...
    <nav class="breadcrumb has-bullet-separator is-centered" aria-label="breadcrumbs">
        <ul>
            <li class="is-active">
            <a href="#" aria-current="page"> <?php echo $Manifest->{'name'}." - V ".$Manifest->{'version'}; echo " - CIDADE"; ?></a>
            </li>
    ...
    <?php
        ...
        $query_ramal = mysqli_query($link, "SELECT DISTINCT cidade FROM sis_cliente");
        $cidade_cliente["%"] = "TODOS";
        while ($lista_ramal = mysqli_fetch_array($query_ramal)){
            $cidade_cliente[$lista_ramal['cidade']] = $lista_ramal['cidade'];
    ...
    <table id="table_busca">
        <tr>
            <td colspan="4"><b>Gráfico de CIDADE é baseado na cidade no cadastro do cliente.</b></td>
        </tr>
        <tr>
        ...
        <tr>
            ...
            <td>
                <select name="cidade">
                <?php
                    foreach ($cidade_cliente as $key => $value) {
                        $selected = ($cidade == $key) ? "selected=\"selected\"" : null;
                        echo "<option value=\"$key\" $selected >$value</option>";
                }
                ?>
                </select>
    ...
    <?php    
          ...
          unset($cidade_cliente['%']); //Retirar item % da Array
/*
    ...
    for($i=$per_ini; $i <= $per_meses; $i++){
        ...
        if($cidade == "%"){
            $query_mes[$i] = mysqli_query($link, "SELECT a.id, a.login, a.datapag, a.valorpag, b.cidade FROM sis_lanc a, sis_cliente b
            WHERE 
            a.login = b.login 
            ...
            a.login = b.login 
                AND a.datapag LIKE '$ano-$i%'
            AND a.login IN (SELECT login FROM sis_cliente WHERE cidade LIKE '$cidade') 
            ORDER BY a.datapag");
        }
