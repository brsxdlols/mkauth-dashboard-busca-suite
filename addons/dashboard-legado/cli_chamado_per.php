        ...
        <?php   
            ...
            for($i=$per_meses-1; $i >= 0; $i--){
                ...
                /*
                 * Para cada período (ano-mês) inicializamos as variáveis de contagem de tickets.
                 * Isso evita que contadores se acumulem indevidamente entre os períodos e
                 * previne erros de variável indefinida no PHP 8 quando incrementamos chaves de array.
                 */
                // zera contadores globais de tickets abertos/fechados para este período
                $cont_ticket_aberto = 0;
                $cont_ticket_fechado = 0;

                // Garante que os arrays de assuntos e técnicos existam com valores iniciais zero
                foreach ($motivo_chamado as $subjectKey => $subjectValue) {
                    if (!isset($assunto_ticket_aberto_[$ano_mes][$subjectValue])) {
                        $assunto_ticket_aberto_[$ano_mes][$subjectValue] = 0;
                    }
                    if (!isset($assunto_ticket_fechado_[$ano_mes][$subjectValue])) {
                        $assunto_ticket_fechado_[$ano_mes][$subjectValue] = 0;
                    }
                    if (!isset($assunto_ticket_saldo_[$ano_mes][$subjectValue])) {
                        $assunto_ticket_saldo_[$ano_mes][$subjectValue] = 0;
                    }
                    // inicializa o total acumulado por assunto quando necessário
                    if (!isset($tot_assunto_ticket_aberto_[$subjectValue])) {
                        $tot_assunto_ticket_aberto_[$subjectValue] = 0;
                    }
                }
                foreach ($nome_tecnico as $techKey => $techName) {
                    if (!isset($tecnico_ticket_aberto_[$ano_mes][$techKey])) {
                        $tecnico_ticket_aberto_[$ano_mes][$techKey] = 0;
                    }
                    if (!isset($tecnico_ticket_fechado_[$ano_mes][$techKey])) {
                        $tecnico_ticket_fechado_[$ano_mes][$techKey] = 0;
                    }
                    if (!isset($tecnico_ticket_saldo_[$ano_mes][$techKey])) {
                        $tecnico_ticket_saldo_[$ano_mes][$techKey] = 0;
                    }
                    // inicializa o total acumulado por técnico (chave pelo nome) quando necessário
                    if (!isset($tot_tecnico_ticket_aberto_[$techName])) {
                        $tot_tecnico_ticket_aberto_[$techName] = 0;
                    }
                }
