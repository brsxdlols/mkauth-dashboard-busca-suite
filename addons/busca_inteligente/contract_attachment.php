...
if (isset($_GET['view'])) {
    ...
    if (!$row || !preg_match('/^[a-f0-9]{48}\.(pdf|jpg|png)$/', $row['storage_name'])) { http_response_code(404); exit('Anexo n?o encontrado.'); }
    $file = $storage . '/' . $row['storage_name'];
    if (!is_file($file) || is_link($file)) { http_response_code(404); exit('Arquivo n?o encontrado.'); }
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    try {
        ...
        foreach (array($start,$end) as $date) {
            ...
            if (!$parsed || $parsed->format('Y-m-d') !== $date) throw new RuntimeException('Informe datas v?lidas.');
        }
        if ($end !== '' && ($start === '' || $end < $start)) throw new RuntimeException('Informe o in?cio e um vencimento igual ou posterior a ele.');
        ...
        if (filesize($upload['tmp_name']) > 10*1024*1024 || filesize($upload['tmp_name']) === 0) throw new RuntimeException('Envie um arquivo de at? 10 MB, n?o vazio.');
        ...
        if (!isset($types[$mime])) throw new RuntimeException('S?o aceitos somente PDF, JPG e PNG.');
        if ($mime !== 'application/pdf' && !@getimagesize($upload['tmp_name'])) throw new RuntimeException('Imagem inv?lida.');
        if (!is_dir($storage) || !is_writable($storage)) throw new RuntimeException('Armazenamento indispon?vel. Execute o instalador atualizado.');
        $name = bin2hex(random_bytes(24)).'.'.$types[$mime];
        if (!move_uploaded_file($upload['tmp_name'], $storage.'/'.$name)) throw new RuntimeException('N?o foi poss?vel guardar o arquivo.');
        ...
        if (!$stmt) { unlink($storage.'/'.$name); throw new RuntimeException('N?o foi poss?vel registrar o anexo.'); }
        mysqli_stmt_bind_param($stmt,'sssssss',$uuid,$name,$original,$mime,$start,$end,$user);
        if (!mysqli_stmt_execute($stmt)) { unlink($storage.'/'.$name); throw new RuntimeException('N?o foi poss?vel registrar o anexo.'); }
...
<?php if ($saved) { ?><p class="success" role="status">Contrato anexado. Ele j? ser? contabilizado no relat?rio.</p><a class="button" href="relcontratos.php" target="_top" onclick="if(window.parent!==window){window.parent.postMessage({type:'mka-content-modal-refresh'},window.location.origin);return false;}">Concluir e atualizar</a>
<?php } else { ?>
...
<small>At? 10 MB, sujeito ao limite do servidor. Envie o documento do contrato j? existente. Este envio n?o ? uma assinatura digital.</small>
<div class="dates"><label>In?cio (opcional)<input type="date" name="start_date" value="<?= mka_contract_escape(isset($_POST['start_date']) ? $_POST['start_date'] : ''); ?>"></label><label>Vencimento (opcional)<input type="date" name="end_date" value="<?= mka_contract_escape(isset($_POST['end_date']) ? $_POST['end_date'] : ''); ?>"></label></div>
<p><small>Sem vencimento informado, ser? contabilizado como contrato anexado sem prazo definido. N?o substitui nem apaga os documentos anteriores.</small></p>
