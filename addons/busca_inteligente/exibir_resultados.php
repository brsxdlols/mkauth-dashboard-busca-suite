...
$searchStats=array(
 array('Total',$statsTotal,'100,00%','index.php','is-primary'),array('Adicionais',$stats['additional'],$percent($stats['additional'],$statsTotal),'?busca=adicionais','is-light'),array('Livres',$stats['free'],$percent($stats['free'],$stats['clients']),'?busca=','is-info'),array('Observação',$stats['observation'],$percent($stats['observation'],$stats['clients']),'?busca=obs','is-observation'),array('Bloqueados',$stats['blocked'],$percent($stats['blocked'],$stats['clients']),'?busca=bloq','is-danger'),array('Atraso',$stats['late'],$percent($stats['late'],$stats['clients']),'?busca=atrasado','is-warning'),array('Online',$stats['online'],$percent($stats['online'],$statsTotal),'?busca=on','is-success'),array('Offline',$stats['offline'],$percent($stats['offline'],$statsTotal),'?busca=off','is-dark'),array('Sem carnê',$stats['no_booklet'],$percent($stats['no_booklet'],$statsTotal),'?busca=sem+carne','is-outline-danger'),array('Sem títulos',$stats['no_titles'],$percent($stats['no_titles'],$statsTotal),'?busca=sem+tit','is-outline-danger'),array('Bloq. manual',$stats['manual'],$percent($stats['manual'],$stats['clients']),'?busca=bloqueado+manualmente','is-manual')
);
?>
    <div class="search-stat-grid no_print" aria-label="Resumo de clientes">
        <?php foreach($searchStats as $stat){ ?><a class="search-stat-card <?= $stat[4]; ?>" href="<?= $stat[3]; ?>"><span class="search-stat-label"><?= htmlspecialchars($stat[0],ENT_QUOTES,'UTF-8'); ?></span><strong class="search-stat-value"><?= (int)$stat[1]; ?></strong><span class="search-stat-percent"><?= $stat[2]; ?></span></a><?php } ?>
    </div>
