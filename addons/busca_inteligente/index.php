...
<body class="">
    ...
    <style>
        ...
        .search-stat-grid { display:flex; flex-wrap:wrap; justify-content:center; gap:10px; margin:0 15px 12px; padding:12px; border:1px solid #d7e1eb; border-radius:16px; background:#f7f9fc; box-shadow:0 7px 20px rgba(15,23,42,.05); }
        .search-stat-card { --accent:#2563a8;--soft:#edf5ff;position:relative;display:grid;grid-template-columns:38px 1fr auto;align-items:center;gap:10px;flex:0 1 calc((100% - 50px)/6);min-width:180px;min-height:66px;overflow:hidden;padding:10px 12px;border:1px solid color-mix(in srgb,var(--accent) 25%,#d5dee8);border-top:3px solid var(--accent);border-radius:11px;background:linear-gradient(135deg,var(--soft) 0%,color-mix(in srgb,var(--soft) 42%,white) 55%,#fff 100%);color:var(--accent);text-decoration:none;box-shadow:0 2px 5px rgba(15,23,42,.045);transition:transform .16s ease,box-shadow .16s ease,border-color .16s ease; }
        .search-stat-card:hover { color:var(--accent);transform:translateY(-2px);border-color:var(--accent);box-shadow:0 8px 18px rgba(15,23,42,.12); }
        .search-stat-icon { display:flex;align-items:center;justify-content:center;width:38px;height:38px;border-radius:10px;background:var(--soft);color:var(--accent);font-size:16px; }
        .search-stat-content { display:flex;min-width:0;flex-direction:column;line-height:1.1; }.search-stat-label { margin-top:3px;font-size:10px;font-weight:800;line-height:1.18;text-transform:uppercase; }
        .search-stat-value { color:#10263d;font-size:21px;font-weight:800;line-height:1; }.search-stat-percent { align-self:end;padding:4px 6px;border:1px solid color-mix(in srgb,var(--accent) 18%,white);border-radius:999px;background:var(--soft);font-size:9px;font-weight:800;white-space:nowrap; }
        .search-stat-card.is-primary{--accent:#1264d6;--soft:#e3efff}.search-stat-card.is-light{--accent:#536579;--soft:#edf1f5}.search-stat-card.is-info{--accent:#168ca4;--soft:#dff8fc}.search-stat-card.is-observation{--accent:#168a51;--soft:#e3f8ec}.search-stat-card.is-danger{--accent:#cf304c;--soft:#ffeaee}.search-stat-card.is-warning{--accent:#9a6a00;--soft:#fff3cc}.search-stat-card.is-success{--accent:#178b52;--soft:#e3f6eb}.search-stat-card.is-dark{--accent:#354354;--soft:#e9edf1}.search-stat-card.is-outline-danger{--accent:#d5263f;--soft:#fff0f2}.search-stat-card.is-manual{--accent:#9f2020;--soft:#feeaea}
        @supports not (color:color-mix(in srgb,red,white)){.search-stat-card{border-color:#d7e1eb;background:var(--soft)}.search-stat-percent{border-color:#d7e1eb}}
        @media(max-width:1250px){.search-stat-card{flex-basis:calc((100% - 27px)/4)}}@media(max-width:850px){.search-stat-card{flex-basis:calc((100% - 9px)/2)}}@media(max-width:520px){.search-stat-grid{margin-inline:8px}.search-stat-card{flex-basis:100%;min-width:0}}
