...
mka_manual_block_audit_ensure_table(isset($link) ? $link : null);
...
<body class="">
    ...
    <style>
        ...
        .search-stat-grid { display:flex; flex-wrap:wrap; justify-content:center; gap:8px; width:calc(100% - 30px); margin:0 auto 14px; padding:12px; border:1px solid #d7e1eb; border-radius:16px; background:#f7f9fc; box-shadow:0 7px 20px rgba(15,23,42,.05); }
        .search-stat-card { --accent:#2563a8;--card-bg:#2563a8;position:relative;display:flex;flex:0 0 140px;height:122px;flex-direction:column;align-items:stretch;overflow:hidden;padding:10px 11px;border:1px solid var(--accent);border-radius:12px;background:var(--card-bg);color:#fff;text-decoration:none;box-shadow:0 4px 10px rgba(15,23,42,.10);transition:transform .16s ease,box-shadow .16s ease,filter .16s ease; }
        .search-stat-card:hover { color:#fff;transform:translateY(-3px);filter:brightness(1.04);box-shadow:0 10px 22px rgba(15,23,42,.18); }
        .search-stat-card.is-selected { z-index:2;transform:translateY(-4px);filter:brightness(1.04) saturate(1.08);box-shadow:0 0 0 3px #fff,0 0 0 6px #155fc2,0 13px 25px rgba(21,95,194,.26); }
        .search-stat-card.is-selected::after { content:"\2713";position:absolute;right:8px;bottom:7px;display:flex;align-items:center;justify-content:center;width:19px;height:19px;border-radius:50%;background:#fff;color:#155fc2;font-size:11px;font-weight:900;box-shadow:0 2px 6px rgba(15,23,42,.22); }
        .search-stat-card.is-selected .search-stat-percent { padding-right:21px; }
        .search-stat-icon { position:absolute;top:9px;right:9px;display:flex;align-items:center;justify-content:center;width:29px;height:29px;border-radius:8px;background:rgba(255,255,255,.19);color:inherit;font-size:14px; }
        .search-stat-content { display:flex;min-width:0;flex-direction:column;line-height:1.08; }.search-stat-label { order:-1;max-width:94px;margin:1px 0 13px;font-size:10px;font-weight:800;line-height:1.12;text-transform:uppercase; }
        .search-stat-value { color:inherit;font-size:31px;font-weight:400;line-height:1;letter-spacing:-1px; }.search-stat-percent { margin-top:auto;padding-top:7px;border-top:1px solid rgba(255,255,255,.23);font-size:11px;font-weight:800;text-align:center;white-space:nowrap; }
        .search-stat-card.is-primary{--accent:#2472e8;--card-bg:linear-gradient(145deg,#438cff,#1763dc)}.search-stat-card.is-light{--accent:#cbd7e5;--card-bg:#fff;color:#15263a}.search-stat-card.is-info{--accent:#20abc5;--card-bg:linear-gradient(145deg,#48c9df,#20abc5)}.search-stat-card.is-observation{--accent:#67d99a;--card-bg:linear-gradient(145deg,#91e8b6,#69d99a);color:#075d35}.search-stat-card.is-danger{--accent:#e83b58;--card-bg:linear-gradient(145deg,#f35a70,#e83452)}.search-stat-card.is-warning{--accent:#efb400;--card-bg:linear-gradient(145deg,#ffd34d,#f5b900);color:#4d3900}.search-stat-card.is-success{--accent:#198f52;--card-bg:linear-gradient(145deg,#35a86d,#16854b)}.search-stat-card.is-dark{--accent:#202934;--card-bg:linear-gradient(145deg,#3d4651,#20262e)}.search-stat-card.is-outline-danger{--accent:#ef4960;--card-bg:#fff;color:#e42643}.search-stat-card.is-manual{--accent:#b42323;--card-bg:linear-gradient(145deg,#c63b3b,#971b1b)}
        .search-stat-card.is-light:hover,.search-stat-card.is-light.is-selected{color:#15263a}.search-stat-card.is-observation:hover,.search-stat-card.is-warning:hover{color:inherit}.search-stat-card.is-outline-danger:hover,.search-stat-card.is-outline-danger.is-selected{color:#e42643}.search-stat-card.is-light .search-stat-icon{background:#edf2f7}.search-stat-card.is-outline-danger .search-stat-icon{background:#fff0f2}.search-stat-card.is-light .search-stat-percent,.search-stat-card.is-outline-danger .search-stat-percent{border-top-color:currentColor}
        @media(max-width:1650px){.search-stat-card{flex-basis:132px}}@media(max-width:1200px){.search-stat-card{flex-basis:140px}}@media(max-width:560px){.search-stat-grid{width:calc(100% - 16px);padding:10px}.search-stat-card{flex-basis:calc(50% - 4px);min-width:132px}}@media(max-width:350px){.search-stat-card{flex-basis:100%}}
        ...
        .content-view-card { display:flex; flex-direction:column; width:min(1120px,100%); height:min(860px,calc(100vh - 36px)); overflow:hidden; border:1px solid #d8e3ef; border-radius:18px; background:#fff; box-shadow:0 28px 80px rgba(15,23,42,.30); }
        ...
        .smart-state-toast.is-action-unlocked { border-color:#15803d; border-left-color:#14532d; background:linear-gradient(135deg,#22a95b 0%,#168447 100%); box-shadow:0 16px 38px rgba(22,132,71,.30); }
        .smart-state-toast.is-action-trust { border-color:#d69e00; border-left-color:#a16207; background:linear-gradient(135deg,#facc15 0%,#eab308 100%); box-shadow:0 16px 38px rgba(234,179,8,.30); color:#3f3000; }
        .smart-state-toast.is-action-trust .smart-state-toast-label,.smart-state-toast.is-action-trust .smart-state-toast-title,.smart-state-toast.is-action-trust .smart-state-toast-meta { color:#3f3000; }
        .smart-state-toast.is-action-trust .smart-state-toast-label { color:rgba(63,48,0,.72); }
        .smart-state-toast.is-action-trust .smart-state-toast-icon { color:#3f3000; background:rgba(255,255,255,.30); }
        ...
        .smart-state-toast-content { min-width:0; flex:1; display:grid; grid-template-columns:minmax(210px,1.2fr) minmax(240px,1fr) auto; align-items:center; gap:10px 20px; }.smart-state-identity{min-width:0;overflow:hidden}.smart-state-context{display:flex;align-items:center;gap:12px;min-width:0;overflow:hidden}.smart-state-toast-label { color:rgba(255,255,255,.78); font-size:9px; font-weight:800; letter-spacing:.07em; text-transform:uppercase; white-space:nowrap; }.smart-state-toast-title { display:block;max-width:100%;overflow:hidden;text-overflow:ellipsis;margin:0; color:#fff; font-size:13px; font-weight:750; white-space:nowrap; }.smart-state-toast-description { margin:0; color:#fff; font-size:10px; white-space:nowrap; }
        .smart-state-toast-meta { display:flex; width:100%; flex-direction:column; align-items:flex-start; flex-wrap:nowrap; gap:2px; color:#fff; font-size:10px; white-space:nowrap; overflow:hidden; }.smart-state-toast-meta span { display:flex; align-items:center; gap:6px; width:100%; overflow:hidden; text-overflow:ellipsis; white-space:nowrap; }.smart-state-toast-meta span i{flex:0 0 auto}
    ...
    (function () {
        ...
        form.addEventListener('submit',function(event){
            ...
            fetch('client_manual_block_audit.php',{method:'POST',credentials:'same-origin',headers:{'Content-Type':'application/x-www-form-urlencoded;charset=UTF-8'},body:body.toString()}).then(function(response){if(!response.ok)throw new Error('Não foi possível registrar a autorização da operação.');return response.json();}).then(function(){return fetch(nativeUrl,{credentials:'same-origin',cache:'no-store'});}).then(function(response){if(!response.ok)throw new Error('Falha ao executar a operação nativa.');return response.text();}).then(function(text){if(/acesso negado/i.test(text))throw new Error('A sessão não possui permissão para executar esta operação.');window.location.reload();}).catch(function(error){waiting=false;submit.disabled=false;note.textContent=error.message||'Não foi possível concluir a operação.';});
        });
    ...
    (function () {
        ...
        function showStateToast(data) {
            ...
            var item=document.createElement('div'); item.className='smart-state-toast is-action-'+clean(data.action_style || 'unlocked');
            item.innerHTML='<button type="button" class="smart-state-toast-close" aria-label="Fechar">&times;</button>'+
                ...
                '<div class="smart-state-identity"><span class="smart-state-toast-label">'+clean(data.label)+'</span><p class="smart-state-toast-title">'+clean(data.name)+'</p></div>'+
                '<div class="smart-state-context"><div class="smart-state-toast-meta"><span><i class="bi bi-person-badge"></i>'+clean(data.login)+'</span><span><i class="bi bi-clock"></i>'+clean(data.formatted_time)+'</span></div></div>'+
