...
<body class="">
    ...
    (function () {
        ...
        function applyClientReportLayout(){try{var doc=frame.contentDocument;if(!doc)return;if(clientUuid&&!doc.getElementById('mka-client-report-layout')){var style=document.createElement('link');style.id='mka-client-report-layout';style.rel='stylesheet';style.href='/admin/addons/busca_inteligente/client_pdf.css?v=7';doc.head.appendChild(style);var zoom=document.createElement('style');zoom.id='mka-client-report-screen-zoom';zoom.textContent='@media screen{html,body{font-size:22.4px!important;line-height:1.42!important}td,th{font-size:21.6px!important;padding:9px 12px!important}h1{font-size:49px!important}h2{font-size:36px!important}h3,h4{font-size:26px!important}}';doc.head.appendChild(zoom);}Array.prototype.forEach.call(doc.images,function(img){if(/img_nao_disp\.gif(?:\?|$)/i.test(img.src||'')){img.src='/mkfiles/logo.jpg';img.alt='Logo do provedor';img.style.objectFit='contain';}});loadRecentConnections(doc);}catch(e){}}
        ...
        pdfBtn.addEventListener('click',function(){if(!clientUuid)return;pdfBtn.disabled=true;Promise.resolve(connectionsPromise).catch(function(){}).then(function(){var doc=frame.contentDocument;if(!doc||!doc.documentElement)throw new Error('As informações do cliente ainda não carregaram.');var data=new FormData();data.append('uuid',clientUuid);data.append('report_html','<!DOCTYPE html>\n'+doc.documentElement.outerHTML);return fetch('client_pdf.php',{method:'POST',credentials:'same-origin',body:data});}).then(function(response){if(!response.ok)return response.text().then(function(message){throw new Error(message||'Não foi possível gerar o PDF.');});return response.blob();}).then(function(blob){var url=URL.createObjectURL(blob),anchor=document.createElement('a');anchor.href=url;anchor.download='dados_cliente.pdf';document.body.appendChild(anchor);anchor.click();anchor.remove();window.setTimeout(function(){URL.revokeObjectURL(url);},2000);}).catch(function(error){window.alert(error.message||'Não foi possível gerar o PDF.');}).then(function(){pdfBtn.disabled=false;});});
